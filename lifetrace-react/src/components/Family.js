  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="max-w-3xl mx-auto p-4 sm:p-6">
        ...
        <p className="text-sm mb-4 text-gray-700">连接家人，彼此见证，共同守护家族的记忆。</p>
        <p className="mb-4">我的UID：{uid || '获取中…'}</p>
      <div className="card p-4 mb-4" style={{ background: 'linear-gradient(135deg, #dbeafe 0%, #ffffff 50%)', borderColor: '#e5e7eb' }}>
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          <button className="btn btn-primary" onClick={sendRequest}>发送请求</button>
        </div>
      </div>
      ...
      <div className="card p-4 mb-4" style={{ background: 'linear-gradient(135deg, #eff6ff 0%, #ffffff 60%)', borderColor: '#e5e7eb' }}>
        ...
        {familyRequests && familyRequests.length > 0 ? familyRequests.map(r => (
          <div key={r.id} className="flex items-center gap-2 mb-2">
            ...
            <button className="btn btn-primary" onClick={() => acceptRequest(r.id)}>接受</button>
            <button className="btn btn-secondary" onClick={() => rejectRequest(r.id)}>拒绝</button>
          </div>
        )) : <p>暂无请求</p>}
      </div>
      ...
      <div className="card p-4 mb-4" style={{ background: 'linear-gradient(135deg, #eff6ff 0%, #ffffff 60%)', borderColor: '#e5e7eb' }}>
        ...
        {familyBiographies && familyBiographies.length > 0 ? familyBiographies.map((b) => (
          <div key={b.id} className="flex items-center justify-between border-b py-2" style={{ borderColor: '#e5e7eb' }}>
            <div>
              <div className="font-medium">{b.title || '(无标题)'} <span className="text-sm text-gray-600">（{getOwnerRelationLabel(b.ownerId, b.isOwner)}）</span></div>
              <div className="text-sm text-gray-600">{new Date(b.timestamp).toLocaleString('zh-CN')} | {b.isPublic ? '已公开' : '私有'}</div>
            </div>
            <div className="flex gap-2">
              <button className="btn btn-secondary" onClick={() => navigate(`/view/${b.id}`)}>查看</button>
